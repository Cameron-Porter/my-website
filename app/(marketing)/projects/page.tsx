    ...
    <section className='mx-auto max-w-[1180px] px-5 py-16 sm:px-6 md:py-24'>
      <div className='max-w-3xl'>
        <div className='flex items-center gap-2'>
          ...
          <p className='text-sm font-semibold uppercase tracking-[0.3em] text-accent-pine'>Field notes</p>
        </div>
      ...
      <div className='mt-12 grid gap-6 lg:grid-cols-3'>
        {projects.map((project) => (
          <Link key={project.slug} href={project.href} className='group flex min-h-[360px] flex-col rounded-[2rem] border border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.045] p-7 transition hover:-translate-y-1 hover:border-accent-ember/30 hover:bg-black/[0.035] dark:hover:bg-white/[0.065]'>
            <span className='patch inline-block w-fit rounded-full border border-accent-pine/30 px-3 py-1 text-xs font-bold uppercase tracking-[0.26em] text-accent-pine'>{project.status}</span>
            ...
            <div className='mt-auto pt-8'>
              ...
              <span className='inline-flex items-center gap-2 text-sm font-semibold text-accent-pine'>Open case study <ArrowRight size={15} /></span>
            </div>
          </Link>
        ))}
      </div>
