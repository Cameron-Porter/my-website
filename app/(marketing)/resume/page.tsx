  ...
  return (
    <section className='mx-auto max-w-[980px] px-5 py-16 sm:px-6 md:py-24'>
      <div className='rounded-[2rem] border border-white/10 bg-white/[0.04] p-7 md:p-10'>
        <p className='text-sm font-semibold uppercase tracking-[0.3em] text-accent-gold'>Resume</p>
        ...
        <div className='mt-8 flex flex-wrap gap-3'>
          <a className='inline-flex h-11 items-center gap-2 rounded-full bg-accent-gold px-5 text-sm font-bold text-primary-bg transition hover:bg-accent-gold/90' href={personalSite.links.linkedin}>
            ...
            <ul className='mt-5 space-y-3'>
              {section.items.map((item) => (
                <li key={item} className='flex gap-3 text-sm leading-6 text-muted-text'>
                  <span className='mt-2 h-2 w-2 shrink-0 rounded-full bg-accent-gold shadow-[0_0_16px_rgba(20,184,166,0.45)]' />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
