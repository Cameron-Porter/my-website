      ...
      <div className='mx-auto grid max-w-[1180px] gap-10 md:grid-cols-[1.4fr_0.7fr_0.7fr_0.7fr] md:gap-8'>
        <div className='space-y-4'>
          ...
          <div className='flex flex-wrap gap-3'>
            ...
            <Link className='rounded-full bg-accent-pine px-4 py-2 text-sm font-semibold text-on-accent transition hover:bg-accent-pine/90' href='/resume'>
              Resume
            </Link>
          </div>
        </div>
        ...
        <ul className='space-y-1'>
          {links.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className='inline-flex min-h-[44px] items-center text-sm text-muted-text transition-colors duration-200 ease-out hover:text-accent-pine md:min-h-0'
              >
                {link.label}
              </Link>
            </li>
